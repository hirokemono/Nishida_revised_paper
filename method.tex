...
%
\begin{eqnarray}
\frac{\partial \vec{u}}{\partial t} + \left(\boldsymbol{\omega} \times \vec{u}\right)
 &=& - \bnabla \left(P+\frac{1}{2}u^{2} \right) - \bnabla \times \bnabla \times \vec{u}
\nonumber \\
 & &   - \frac{2}{E} \left(\hat{\vec{z}} \times \vec{u} \right)
  + \frac{Ra}{Pr} T \frac{\vec{r}}{r_{o}}
  + \frac{1}{Pm E} \left(\vec{J} \times \vec{B} \right),
\label{eq:momentum} \\
%
\bnabla \bcdot \vec{u} &=& 0, 
\label{eq:conservation} \\
%
\frac{\partial T}{\partial t} + \left(\vec{u} \bcdot \bnabla \right) T
 &=& \frac{1}{Pr} \bnabla^{2} T,
\label{eq:heat} \\
%
 \frac{\partial \vec{B}}{\partial t}
 &=& -\frac{1}{Pm}  \bnabla \times \bnabla \times \vec{B}
       + \bnabla \times \left(\vec{u} \times \vec{B} \right),
\label{eq:induction}
\end{eqnarray}
%
and
\begin{equation}
\bnabla \bcdot \vec{B} = 0,
\label{eq:Gauss_B}
\end{equation}
%
where $\vec{u}$, $t$, $P$, $T$, $\vec{B}$, $\vec{r}$, and $\hat{\vec{z}}$ are the velocity, time, reduced pressure, temperature, magnetic field, position vector, and unit vector along the rotation axis, respectively.
In eq.~(\ref{eq:momentum}), $\boldsymbol{\omega} = \bnabla \times \vec{u}$ is the vorticity, and $\vec{J} = \mu_{0}^{-1} \bnabla \times \vec{B}$ in the dimensional form is the current density, where $\mu_0$ is the magnetic permeability of vacuum.
...
We use a numerical dynamo code Calypso \citep{Kemo:2014}, in which numerical simulations are carried out in the spherical coordinates, $(r, \theta, \phi)$.
Solenoidal vector fields, $\vec{u}$ and $\vec{B}$, are decomposed into the toroidal and poloidal constituents as
%{\color{red}
%
\begin{equation}
\vec{u}(\vec{r}, t) = \bnabla \times (u_T (\vec{r}, t) \hat{\vec{r}} ) + \bnabla \times \bnabla \times (u_S (\vec{r}, t) \hat{\vec{r}} ) ,
\label{eq:u_uT_uS}
\end{equation}
%
\begin{equation}
\vec{B}(\vec{r}, t) = \bnabla \times (B_T (\vec{r}, t) \hat{\vec{r}} ) + \bnabla \times \bnabla \times (B_S (\vec{r}, t) \hat{\vec{r}} ) ,
\label{eq:B_BT_BS}
\end{equation}
%
where $\hat{\vec{r}}$ is the radial unit vector.
Todoidal and poloidal scalar functions for the velocity and magnetic fields, $u_T(\vec{r}, t)$, $u_S(\vec{r}, t)$, $B_T(\vec{r}, t)$, and $B_S(\vec{r}, t)$ are expanded into spherical harmonics.
As an example, $u_T (\vec{r}, t)$ is expanded as
%
\begin{equation}
u_T (\vec{r}, t) = \sum_{l = 1}^{L_{\rm max}} \sum_{m=-l}^{l} u_{T l}^{\ m} (r, t) Y_l^{|m|} (\theta, \phi) ,
\label{eq:uT_Ylm}
\end{equation}
%
...
The temperature, $T(\vec{r}, t)$, is also expanded into spherical harmonics as
%
\begin{equation}
T (\vec{r}, t) = \sum_{l = 0}^{L_{\rm max}} \sum_{m=-l}^{l} T_l^m(r, t) Y_l^{|m|} (\theta, \phi) .
\label{eq:T_Ylm}
\end{equation}
%
%}
...
The initial magnetic field is set as an axial dipole field and zonal toroidal field with degree $l=2$ adopted by \citet{Uli:2001}. 
...
The mantle and inner core are assumed to be co-rotating, and a non-slip boundary ($\vec{u} = 0$) is applied to the CMB and ICB. 
...
Temporal resolutions and computational duration are found to be sufficient through evaluation of temporal variations in kinetic and magnetic energies.
