...
% We first estimate the critical Rayleigh number, $Ra_{\rm crit}$, for the onset of thermal convection following the method by Al-Shamali {\it et al.} \citep{Al-Shamali:2004}.
We carry out numerical simulations of thermal convection by solving eqs (\ref{eq:momentum})--(\ref{eq:heat}) without the Lorentz force term, $\left(Pm E\right)^{-1} \left(\vec{J} \times \vec{B} \right)$.
The kinetic energy density defined by
%
\begin{equation}
E_{\rm kin} = \frac{1}{V_{S}} \int_{V_{S}} \frac{1}{2} \vec{u}^{2} dV,
...
\input{table_Rac_Ek4}
%
The critical Rayleigh numbers for $E = 10^{-3}$ are estimated as $Ra_{\rm crit} = 1.09 \times 10^5$, $0.72 \times 10^5$ and $0.56 \times 10^5$ for $r_i/r_o = 0.15$, $0.25$ and $0.35$, respectively, and those for $E = 10^{-4}$ are $Ra_{\rm crit} = 1.08 \times 10^6$, $0.82 \times 10^6$ and $0.70 \times 10^6$, respectively.
%
The obtained values of $Ra_{\rm crit}$ are almost identical to those reported in Al-Shamali {\it et al.}\cite{AlShamali:2004} for the same parameters and conditions used in this study. 
%
...
%
\begin{equation}
E_{\rm mag} = \frac{1}{V_{S}E Pm} \int_{V_{S}} \frac{1}{2} \vec{B}^{2} dV,
...
\begin{equation}
E_{\rm mag} (r=r_o) = 
  \frac{1}{S_o E Pm} \int_{S_o} \frac{1}{2} \vec{B}^2 dS,
\end{equation}
%
...
The dynamic Elsasser number, $\Lambda_d$, represents the relative strength of the Lorentz to Coriolis forces defined by Soderlund {\it et al.} \citep{Soderlund:2012}.
...
This is consistent with a result of Heimpel {\it et al.} \citep{Heimpel:2005}, where all the $E_{\rm mag}$ values are also smaller than the $E_{\rm kin}$ values at $E = 4.0 \times 10^{-4}$ and $Pm = 5$ for $r_i / r_o = 0.15$.
